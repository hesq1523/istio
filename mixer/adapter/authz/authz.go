...
func (h *handler) checkPermission(inst *authorization.Instance) (bool, error) {
	...
	if len(token) != 0 {
		...
		resourceURL := fmt.Sprintf("http://%s.%s%s?method=%s&resource_url=%s", parts[0], parts[1], path, method, path)
		...
		logger.Infof(fmt.Sprintf("[begin request]URL: %s", payload.ResourceURL))
		...
		logger.Infof(fmt.Sprintf("[end request]URL: %s, Status code: %s", payload.ResourceURL, resp.Status))
...
func GetInfo() adapter.Info {
	return adapter.Info{
		...
		DefaultConfig: &config.Params{
			ServiceUrl:    "http://hp-authz-service.hp/authz/v1/authorize",
